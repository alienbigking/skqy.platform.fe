...
import styles from './productManagementEdit.less'
...
import { EShelfStatus, EUrgencyType } from '@/pages/common/types/common'
...
const ProductManagementEdit: React.FC<Props> = memo((props) => {
  ...
  const getDetail = async () => {
    const { code, data } = await productManagementService.getDetail(rowData.id)
    console.log('获取的商品详情', data)
    if (code === '200') {
      handleEditData(data)
    }
  }
  ...
  const handleEditData = (values: any) => {
    ...
    values.status = values.status === EShelfStatus.listed
    values.urgencyType = values.urgencyType === EUrgencyType.express
    // 初始化选中的类别 code
    setSelectedCategoryCode(values.category?.code)

    values.externalField = {
      externalFieldKey: values?.productExternalRelevanceInfo
        ? [Object.keys(values?.productExternalRelevanceInfo)[0]]
        : '',
      externalFieldValue: values?.productExternalRelevanceInfo
    ...
    fileOssIdRef.current.urls = values.productImages
    const images = values.productImages
      ?.map((item: any, key: number) => {
        let value = parseUrlToFileInfo(item)
        console.log('解析后图片的数据', value)
        // 如果 value 不为 null，则可以添加 status 属性
        if (value) {
          value['uid'] = key
          value['status'] = 'done'
        }
    ...
    productDetailImagesOssIdRef.current.urls = values?.productDetailImages || []
    const productDetailImages = values?.productDetailImages
      ?.map((item: any, key: number) => {
        let value = parseUrlToFileInfo(item)
        // 如果 value 不为 null，则可以添加 status 属性
        if (value) {
          console.log('详情图片数组', value)
          value['uid'] = key
          value['status'] = 'done'
        }
    ...
    values.files = images || []
    values.productDetailFiles = productDetailImages || []

    console.log('编辑时的主图内容', images)
    console.log('详情图片数组', productDetailImages)
    console.log('编辑内容', values)
  ...
  const onOk = async () => {
    ...
    console.log('上传的主图结果', {
      urls: fileOssIdRef.current.urls,
      productImages: productImages
    })

    console.log('上传的详情图片结果', {
      urls: productDetailImagesOssIdRef.current.urls,
      productDetailImages: productDetailImages
    })

    const data = await productManagementService.update(rowData.id, {
      ...values,
      productImages: productImages,
      productDetailImages: productDetailImages,
  ...
  return (
    <Modal
      className={cn(styles.productManagementEdit)}
      title="编辑商品"
      open={isVisible}
      width={568}
      centered
