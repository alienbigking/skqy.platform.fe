...
import styles from './interpretationManagementEdit.less'
...
const InterpretationManagementEdit: React.FC<Props> = memo((props) => {
  ...
  const getDetail = async () => {
    const { code, data } = await interpretationManagementService.getDetail(
      rowData.id
    )
  ...
  const onOk = async () => {
    ...
    const data = await interpretationManagementService.update(rowData.id, {
      ...values
    })
    if (data.code === '200') {
...
export default InterpretationManagementEdit
