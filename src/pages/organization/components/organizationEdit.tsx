...
import { isNullObject, uploadNoticeOptions } from '@/utils'
...
const OrganizationEdit: React.FC<Props> = memo((props) => {
  ...
  const getDetail = async () => {
    const { code, data } = await organizationService.getDetail(rowData.id)
    console.log('获取的机构详情', data)
    if (code === '200') {
      handleEditData(data)
    }
  }
  ...
  const handleEditData = (values: any) => {
    ...
    console.log('当前的上级机构ID', values.pid)
  ...
  const onOk = async () => {
    ...
    const { code } = await organizationService.update({
      id: rowData.id,
      ...values,
  ...
  return (
    <Modal
      className={cn(styles.organizationEdit)}
      title="编辑机构"
      open={isVisible}
      width={568}
      centered
        ...
        <Row gutter={24}>
          ...
          <Col span={24}>
            <Form.Item<FieldType>
              label="数据上传"
              name="uploadNotice"
              rules={[{ required: false, message: '请选择数据上传是否通知' }]}
            >
