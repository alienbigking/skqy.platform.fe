...
import styles from './internalProductNew.less'
...
const InternalProductNew: React.FC<Props> = memo((props) => {
  ...
  const onOk = async () => {
    ...
    const data = await internalProductService.add({ ...values })

    if (data.code === '200') {
      message.success('新增机构产品成功')
      form.resetFields()
      handleOk?.()
    }
  }
...
export default InternalProductNew
