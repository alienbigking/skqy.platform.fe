...
import styles from './submitReportAndAnalysis.less'
import { AdvancedButton } from '@/components/advancedButton'
import { EType } from '@/components/advancedButton/types/advancedButton'
import { isNullObject, positiveLevelResultOptions } from '@/utils'
...
const SubmitReportAndAnalysis: React.FC<Props> = memo((props) => {
  ...
  return (
    <Modal
      className={cn(styles.submitReportAndAnalysis)}
      title="提交动态报告结论"
      open={isVisible}
      width={568}
      onOk={onOk}
    ...
    >
      ...
      >
        <Row gutter={24}>
          ...
          <Col span={24}>
            <Form.Item label="动态报告上传" required>
              <Radio.Group
                className={cn(styles.uploadRadio)}
              ...
              />
              {radioValue === 0 ? (
                <div className={cn(styles.text)}>默认一键自动回传</div>
              ) : (
                <Form.Item<FieldType>
                  name="files"
                  valuePropName="fileList"
                  getValueFromEvent={normFile}
                  rules={[{ required: true, message: '请上传动态报告' }]}
                >
                  <Dragger {...uploadProps}>
...
export default SubmitReportAndAnalysis
